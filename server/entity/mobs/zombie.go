...
func (z *Zombie) Apply(data *world.EntityData) {
	z.mc = &entity.MovementComputer{Gravity: 0.08, Drag: 0.02, StepHeight: 0.6}
	data.Data = z
}

func (z *Zombie) Tick(e *entity.Ent, tx *world.Tx) *entity.Movement {
	z.self = e
	if z.Dead() {
		return nil
	}
	...
	if z.brain == nil {
		...
		wBridge := entity.WorldBridge{E: e}
		z.navigator = mobsx.NewNavigator(entity.EntityBridge{E: e, tx: tx}, wBridge)
	...
	z.brain.Tick(entity.EntityBridge{E: e, tx: tx}, wBridge)
...
func (t zombieType) Open(tx *world.Tx, handle *world.EntityHandle, data *world.EntityData) world.Entity {
	e := &entity.Ent{tx: tx, handle: handle, data: data}
...
func (z *Zombie) Hurt(damage float64, src world.DamageSource) (n float64, v bool) {
	...
	if z.health <= 0 && z.self != nil {
		...
		// XP drops
		for _, handle := range entity.NewExperienceOrbs(z.self.Position(), 5) {
