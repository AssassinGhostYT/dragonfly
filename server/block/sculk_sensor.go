...
// scan checks for entities in an 8-block radius and detects movement.
func (s SculkSensor) scan(tx *world.Tx, pos cube.Pos) bool {
	...
	for e := range tx.EntitiesWithin(cube.Box(
		...
		// Only detect players or entities that are moving (vibrating)
		var vel mgl64.Vec3
		...
		// If entity is in range and moving (or we want high sensitivity)
		if dist <= 8 && (vel.Len() > 0.01 || dist < 1.0) {
...
func (s SculkSensor) detect(tx *world.Tx, pos cube.Pos, origin mgl64.Vec3, e world.Entity) {
	...
	// Particle at the destination (sensor) with origin data in the packet
	...
	// Stay on for 1.5 seconds (30 ticks)
