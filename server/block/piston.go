...
// SideClosed ...
func (Piston) SideClosed(cube.Pos, cube.Pos, *world.Tx) bool {
	return false
}
...
func (p Piston) DecodeNBT(m map[string]any) any {
	...
	p.Sticky = nbtconv.Uint8(m, "Sticky") == 1
	return p
}
...
func (p Piston) push(pos cube.Pos, tx *world.Tx) bool {
	if p.State == 0 {
		...
		for _, breakPos := range resolver.breakPositions {
			...
			if b, ok := tx.Block(breakPos).(interface {
				BreakInfo() BreakInfo
			}); ok {
		...
		for _, attachedPos := range resolver.attachedPositions {
			...
			updateStrongRedstone(attachedPos, tx)
		}

		p.State = 1
		tx.SetBlock(pos.Side(face), PistonArmCollision{Facing: p.Facing, Sticky: p.Sticky}, nil)
	} else if p.State == 1 {
		if p.Progress >= 1 {
			p.State = 2
		}
...
func (p Piston) pull(pos cube.Pos, tx *world.Tx) bool {
	if p.State == 2 {
		...
		pullDir := face.Opposite()
		for _, attachedPos := range resolver.attachedPositions {
			side := attachedPos.Side(pullDir)
			p.AttachedBlocks = append(p.AttachedBlocks, attachedPos)
